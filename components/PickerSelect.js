...
export default class PickerSelect extends PureComponent {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        const items = PickerSelect.handlePlaceholder({
            placeholder: nextProps.placeholder,
        }).concat(nextProps.items);
        const itemsChanged = !isEqual(prevState.items, items);

        // update selectedItem if value prop is defined and differs from currently selected item
        const { selectedItem, idx } = PickerSelect.getSelectedItem({
            items,
    ...
    renderIOS() {
        ...
        return (
            <View style={[defaultStyles.viewContainer, style.viewContainer,]}>
                ...
                >
                    <TouchableOpacity
                        style={[defaultStyles.modalViewTop, style.modalViewTop, {backgroundColor:'#000', opacity: .5}]}
                        onPress={() => {
                            this.togglePicker(true);
                        }}
                    />
